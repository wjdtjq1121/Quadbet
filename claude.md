...
### v1.8.7 (2025-01-02)
- **1-card 버그 완전 해결**
  - isValidPlay 함수에 playerHandSize 파라미터 추가
  - 마지막 카드일 때 특별 규칙 적용 (>= 비교 허용)
  - 전역 변수 의존성 제거로 안정성 향상
  - 마지막 카드를 낼 수 없던 문제 완전 해결
- **모바일 UI 버튼 위치 고정**
  - game-action-buttons가 스크롤을 따라가는 문제 수정
  - 중앙 영역 하단에 고정된 위치로 변경
  - 모바일 사용성 대폭 개선

### v1.7.3 (2025-12-30)
- **조커 카드 크기 확대**
  - 조커(🃏) 카드가 카드 내에서 더 크게 표시 (font-size: 4em)
  - 카드 내 공간을 꽉 채우도록 스타일링 개선
- **소원 선택 UI 개선**
  - prompt 대신 버튼 기반 모달 UI로 변경
  - 2~10, J, Q, K, A 총 13개 버튼으로 직관적 선택
  - "소원 없이 진행" 옵션 추가
  - showWishModal() 함수로 Promise 기반 비동기 처리
- **소원 카드(1) 보드 표시 문제 수정**
  - 소원 카드가 보드에서 빈 카드로 표시되던 문제 해결
  - renderCard() 함수에서 '1' 카드 특별 스타일링 추가
  - 크고 굵은 "1" 표시 + "소원" 라벨 추가
- **게임 종료 조건 변경**
  - 기존: 3명 완료 시 게임 종료
  - 변경: 한 팀(남북 또는 동서) 완료 시 즉시 게임 종료
  - checkTeamFinished() 헬퍼 함수 추가
  - 팀 1 (플레이어 0, 2), 팀 2 (플레이어 1, 3)
- **폭탄 자동 인식 시스템 구현**
  - 💣 폭탄 버튼 제거 (index.html)
  - playBomb() 함수 제거
  - playCards() 함수에서 폭탄 자동 감지
  - 포카드 또는 스트레이트 플러시 선택 시 자동으로 폭탄 인식
  - 폭탄은 언제든지 낼 수 있음 (턴 무관)
  - 폭탄을 내면 자동으로 턴 획득
...
- 모든 "마작" 용어를 "소원"으로 변경
- 주석 및 로그 메시지 업데이트
- containsMahJong 함수명은 유지 (내부 로직용)
- **magician.png 이미지 적용**
- 불의 정령 아그니 카드에 magician.png 이미지 사용
- 이모지 대신 실제 이미지로 표시
- renderCard() 함수에서 AGNI_IMG 처리
- **페어 밸류 비교 디버깅 로그 추가**
- validateCombination()에 상세 로그
- isValidPlay()에 밸류 비교 로그
- KK vs AA 등 페어 비교 시 디버깅 용이

### v1.7.1 (2025-12-30)
- **마작(숫자 1) 선 플레이어 버그 수정**
- 숫자 1(마작) 보유자가 선 플레이어로 설정되지 않던 문제 수정
- initializeGameState()에서 'One' 카드명 검색 추가 (기존에는 'Mah Jong'만 검색)
- **불의 정령 아그니 이모지 개선**
- 아그니 카드 이모지를 사람 형태로 변경: 🔥✨ → 🧞‍♂️🔥
- 원피스 에이스처럼 신비로운 불의 정령 느낌
- **폭탄(Bomb) 시스템 구현**
- 💣 폭탄 버튼 추가 (화려한 그라데이션 스타일)
- 폭탄은 언제든지 사용 가능 (내 턴이 아니어도 가능)
- Four of a Kind (4장 동일 숫자) 폭탄
- Straight Flush (5장 이상 같은 무늬 연속) 폭탄
- 폭탄 사용 시 즉시 턴 획득
- 폭탄 vs 폭탄: 더 강한 폭탄으로 대응 가능
- playBomb() 함수 구현
- isBomb() 헬퍼 함수 추가
